...
from src.training.callbacks_mlflow import MLflowCallback
from src.training.splitters import WithinSubjectHoldoutSplitter
from src.training.trainer import Trainer_DL
from src.training.utils import accuracy, binarize_y, build_callbacks, create_dataloader, create_dataset
...
def run_training_sweep(
    ...
    for partition in pending:
        ...
        try:
            ...
            # ver PROTOCOL.md). binarize_y no requiere labels numéricos de
            # entrada (mapea cualquier par de valores únicos a 0/1), así
            # que ese paso se omite acá; un dataset que emita labels string
            # sin normalizar es responsabilidad del dataset, no de este
            # orquestador.
            y, bin_to_class = binarize_y(y)

            fold = splitter.split(X, y, metadata, subject_id=subject_id)
            ...
            for model_init_seed in model_init_seeds:
                ...
                run_dir = make_run_dir(strategy_name, recipe_name, database_name, partition, replicate)
                try:
                    ...
                    with mlflow.start_run(run_name=f"{partition}/{replicate}", tags=tags) as run:
                    ...
                    reporter.on_run_result(f"{partition}/{replicate}", "success")
                except Exception as e:
                    logger.error("Fallo entrenando %s/%s: %s", partition, replicate, e)
                    partition_status = "failed"
                    reporter.on_run_result(f"{partition}/{replicate}", "failed")
